// Schema as code. Every statement is idempotent (IF NOT EXISTS), so it can run
// on first request with zero migration tooling. Keep in sync with
// migrations/0001_init.sql (kept for local/reference use).
...
export const SCHEMA_STATEMENTS: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS leads (
     ...
     email TEXT,
     number_choice TEXT,
     ai_recharge_amount INTEGER,
     ai_tier_name TEXT,
     service_type TEXT NOT NULL DEFAULT 'personal',
     notes TEXT,
  ...
  // Live presence (active-now counter). Rows are pruned on read + by cron.
  ...
  // Upload index = the self-tracked storage ledger. One row per stored object.
  `CREATE TABLE IF NOT EXISTS uploads (
     key TEXT PRIMARY KEY,
     kind TEXT NOT NULL,
     bytes INTEGER NOT NULL DEFAULT 0,
     store TEXT NOT NULL DEFAULT 'r2',
     lead_id TEXT,
     created_at TEXT NOT NULL DEFAULT (datetime('now'))
   )`,
  ...
  // Simple all-time counters (visitors table is capped, so totals live here).
